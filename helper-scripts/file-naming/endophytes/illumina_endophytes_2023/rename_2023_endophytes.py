...
if TEST_MODE:
    ...
    args = {
        'input': Path('/Users/carolyndelevich/main/github_repos/climush/helper-scripts/file-naming/endophytes/test-files_2023-sequence-files/'),
        ...
        'output': Path('/Users/carolyndelevich/main/github_repos/climush/helper-scripts/file-naming/endophytes/test-output_2023-sequence-files/'),
        'delim': '-',
        'no_copy': False,
    }
    ...
    parser = argparse.ArgumentParser(prog=Path(__file__).stem,
                                     description='Rename the 2023 endophyte ITS1 sequence files.',
    ...
    parser.add_argument('-c', '--config',
                        required=True,
                        type=pathlib.PosixPath,
                        help='The path to the .toml configuration file for 2023 endophyte renaming.')

    parser.add_argument('-m', '--mapping',
                        required=True,
                        type=pathlib.PosixPath,
                        help='The path to the CTAB code / bag label mapping file for 2023 endophytes.')
...
# instead of using the sequencing platform as the prefix (e.g., illumina) use the gene region ('its1')
# seq_region_prefix = 'its1'

# use sequence platform as a prefix to be consistent with all other sequence collections
platform_prefix = 'illumina'
...
suppress_warnings = {
    ...
    'unexpected_isosource': False,   # the isolation source for a sample isn't among the expected values
    'missing_coll_date': True,      # missing collection date found, fixed following warning
    'nonuniform_coll_date': True,   # a subplot has various collection dates (YYYY-MM) associated with it
    'multiple_domains': True,       # a state abbreviation has multiple NEON domains associated with it
    'missing_old_site': True,       # a NEON domain doesn't have an old climush site name associated with it
    'unrecog_old_site': True,       # an old site name from a sample ID in the dataframe isn't recognized within NEON convert dict
}
...
endo_df.rename(columns={'Code': 'ctab_code',
                        'BAG LABEL': 'original_bag_label'},
               inplace=True)

# fix any typos with the CTAB code prefix, which should be MSC (all here are MCSC, which doesn't match seq files)
endo_df['ctab_code'] = [re.sub(ANY_CTAB_PREFIX, CORRECT_CTAB_PREFIX, c) for c in endo_df['ctab_code']]
...
duplicate_ctab = endo_df[endo_df['ctab_code'].duplicated(keep=False)].sort_values('ctab_code')  # keep=False to return all duplicate rows
...
if (duplicate_ctab.shape[0] == 0) or (suppress_warnings['duplicated_ctabs']):
    ...
    raise KeyboardInterrupt(
        ...
        f'   {set(duplicate_ctab["ctab_code"].to_list())}')

# identify any bag labels that deviate from a somewhat conventional format
label_component_counts = []
for original_label in endo_df['original_bag_label']:
    label_component_counts.append(len(original_label.split(args['delim'])))

mean_component_count = np.mean(label_component_counts)
sd_component_count = np.std(label_component_counts)
...
unique_site_labels = set(label.split('-')[0] for label in endo_df['original_bag_label'])
...
for site in per_site_label_example:
    per_site_label_example.update({site:endo_df[endo_df['original_bag_label'].str.startswith(site)]['original_bag_label'].iloc[0]})
...
unique_labels(endo_df['original_bag_label'], site='BNZ', label_position=4)
...
for site in rename_settings.keys():
    ...
    print('  burn history:')
    unique_burnhist, errors_burnhist = unique_labels(endo_df['original_bag_label'], site=site,
    ...
    print(f'  habitat:')
    unique_habitat, errors_habitat = unique_labels(endo_df['original_bag_label'], site=site,
...
endo_df['sequence_region'] = platform_prefix
...
isolate_sources = {s:(s.split('-')[-1]) for s in endo_df['original_bag_label']}
...
iso_src_errors = {}
for og_bag, iso_src in isolate_sources.items():
    if iso_src in isolate_source_dict:
        pass
    else:
        iso_src_errors.update({og_bag: iso_src})
        if suppress_warnings['unexpected_isosource']:
            pass
        else:
            raise KeyboardInterrupt(f'Isolation source {iso_src} is not an expected value.')

# check what errors occurred
print(f'The following original labels had unexpected isolation source values:\n')
for og_bac, iso_src_err in iso_src_errors.items():
    print(f'   {og_bac}    {iso_src_err}')
...
for old_id in endo_df['original_bag_label']:
...
missing_date_sample = endo_df.loc[missing_date_rows,:]['original_bag_label'].iloc[0]  # get the sample ID of missing date sample
...
common_subplot_colldates = endo_df[endo_df['original_bag_label'].str.match(f'^{missing_subplot_info}') & endo_df['collection_date'].notna()]['collection_date']
...
# common_subplot_colldates = endo_df[endo_df['original_bag_label'].str.match(f'^{missing_subplot_info}') & endo_df['collection_date'] != missing_current_date]['collection_date']
...
if len(set(common_subplot_colldates)) == 1:
    ...
    subplot_df = endo_df[endo_df['original_bag_label'].str.match(f'^{missing_subplot_info}')]
...
for state_abbr in neon_domains:
    ...
    if len(n_dom_list) == 1:
        n_dom = n_dom_list[0]
    elif suppress_warnings['multiple_domains']:
        pass
    else:
...
def get_treatment(bag_label, config_dict, label_delim=args['delim']):
    ...
    # define subfunctions, which create a new label based on whether treatment is single string or multiple
    def single_string():
        '''
        do this if the burn history and habitat are together in a single string, i.e.,
        not separated by a delimiter
        :return: the new treatment label
        '''

        # the habitat and burn history are together in one string, so need to only pull one item from list
        og_treatment_label = label_list[config_dict[site_id]['treatment']['burnhist']['position']]

        # set a search boolean for burn history and habitat, so there's an alert if no match is found
        # otherwise, the variable new_burnhist_label and / or new_habitat_label won't be defined,
        #    could be confusing for troubleshooting
        burnhist_found = False
        habitat_found = False

        # search for a match to one of the possible original burn history labels, and return the corresponding new label
        for bh_og, bh_new in config_dict[site_id]['treatment']['burnhist']['format'].items():
            if re.search(bh_og, og_treatment_label, re.I):
                burnhist_found = True
                new_burnhist_label = bh_new
            else:
                continue

        # search for a match to one of the possible original habitat labels, and return the corresponding new label
        for hb_og, hb_new in config_dict[site_id]['treatment']['habitat']['format'].items():
            if re.search(hb_og, og_treatment_label, re.I):
                habitat_found = True
                new_habitat_label = hb_new
            else:
                continue

        if burnhist_found and habitat_found:

            return new_burnhist_label + new_habitat_label

        else:
            if burnhist_found and not habitat_found:
                issue_label = 'Habitat'
            elif habitat_found and not burnhist_found:
                issue_label = 'Burn history'
            else:
                issue_label = 'Both burn history and habitat'

            err_msg = f'{issue_label} not located within the original bag label: {bag_label}'

            raise KeyboardInterrupt(err_msg)

    def multiple_strings():
        '''
        do this if the original treatment label is split by the delimiter into separate
        burn history and habitat strings
        :return: the new treatment label
        '''
    ...
    # if the burn history and habitat positions are the same, then the original treatment label is a single string
    if config_dict[site_id]['treatment']['burnhist']['position'] == config_dict[site_id]['treatment']['habitat']['position']:
        return single_string()
    elif site_id == 'SRER':
        return multiple_strings()
    else:
        return multiple_strings()
...
new_treatment_dict = {}
error_labels = []
for old_label in endo_df['original_bag_label']:

    # skip over SRER
    # if not old_label.startswith('SRER'):

    # only do SRER
    if old_label.startswith('SRER'):
...
# inspect SRER samples
srer_old_ids = endo_df[endo_df['original_bag_label'].str.startswith('SRER')]['original_bag_label'].to_list()
srer_old_ids.sort()
for id in srer_old_ids:
    print(id)

# iterate through all sites and calculate the length of the split label list
site_ids = set(label.split(args['delim'])[0] for label in endo_df['original_bag_label'])
site_ids = list(site_ids)
site_ids.sort()
for site in site_ids:
    label_list_len = set()
    site_old_ids = endo_df[endo_df['original_bag_label'].str.startswith(site)]['original_bag_label'].to_list()
    for s in site_old_ids:
        label_list_len.add(len(s.split(args['delim'])))

    print(f'{site} = {label_list_len}')
...
old_subplots = list(set(s.split('-')[-3] for s in endo_df['original_bag_label']))  # make list, easier to check if sorted
old_subplots.sort()
...
subplot_col = []
for samp in endo_df['original_bag_label']:
...
file_rename_dict = {old_name: new_name for old_name, new_name in zip(endo_df['ctab_code'], endo_df['updated_sample_id'])}
...
ctab_sample_no = [ re.search(r'(\d+)', c).group(0) for c in endo_df['ctab_code'] if pd.notna(c)]
...
#         ctab_id = endo_df['ctab_code'].iloc[ctab_row_num]
#
...
unlinked_ctab_codes = set(endo_df['ctab_code']).difference(set(matched_ctab_codes))
...
unlinked_old_climush_names = endo_df[endo_df['ctab_code'].isin(unlinked_ctab_codes)]['original_bag_label'].to_list()
...
unlinked_ctab_df = endo_df[endo_df['ctab_code'].isin(unlinked_ctab_codes)]
