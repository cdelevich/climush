...
## adapted from the original version, pulled from location above
...
from climush.utilities import prompt_print_options
import re

## DEFINE PATHS

# approved macrofungi, pulled from the CliMush Google Drive on 2024-10-25; likely to be updated soon
approved_mf_path = Path('/Users/carolyndelevich/main/github_repos/climush/helper-scripts/config/reference-data/macrofungi-defined_2024-10-25.csv')

# test OTU table; used one that I know isn't right, but it is at least small
otu_table_path = Path('/Users/carolyndelevich/main/github_repos/climush/helper-scripts/otu-sorting/isl-2407_otu-tab.txt')


## IMPORT COMMUNITY TAXONOMIC DATA

## needed a sample of a pacbio .fasta file with all of the sequences ID'd for a sample
pb_test_path = Path('/Users/carolyndelevich/main/github_repos/climush/bioinformatics-pipeline/pipeline-output/separated-subregions/itsx_psf2404-2407')

# assign taxonomy based on its1 region only (arbitrary)
region = 'ITS1'

# go through each sample directory, and get path of the ITS1 fasta file
sample_files = []
for sample in pb_test_path.glob('*'):
    for fasta in sample.glob(SEQ_FILE_GLOB):
        if re.search(f'(?<=\.){region}(?=\.)', fasta.name, re.I):
            sample_files.append(fasta)

# check which reference db that amptk should use based on region provided
amptk_ref_dbs = ['ITS1', 'ITS2', 'ITS', '16S', 'LSU', 'COI']
# if the region matches one of the listed reference databases, use this as the reference
if region in amptk_ref_dbs:
    amptk_ref = region
# if it doesn't match, then one has to be specified by user
else:
    amptk_ref = prompt_print_options(option_list=amptk_ref_dbs, auto_respond=False)

# with each file as an input value, run amptk taxonomy on the ITS1 sample with the ITS1 ref db
for file in sample_files:

    # does the sample ID need to be in the header here?

    # compile the command list for running amptk taxonomy on each file
    amptk_cmd = ['amptk', 'taxonomy', '-f', file, '-d', amptk_ref]

# I want to test on a small file since taxonomy is time-consuming
file_sizes = [file.stat().st_size for file in sample_files]
f_sizes = {file_name: file_size for file_name,file_size in zip(sample_files, file_sizes) if file_size > 0}

# get smallest non-zero file size
# min_nonzero_size = min([s for s in f_sizes.values()])
wanted_f_size = 859
# get the file path of the file with this file size
for k,v in f_sizes.items():
    if v == wanted_f_size:
        smallest_file = k

# accept either an OTU table or a .fasta file of sequences, as long as the taxonomic ID is in the header
if otu_table_path.suffix == '.fasta':

    # I want this to be a dictionary, but I'm forgetting how to do that with biopython

    # list of records
    record_list = []

    for record in SeqIO.parse(otu_table_path, '.fasta'):
...
## IMPORT APPROVED MACROFUNGI

approved_macrofungi = pd.read_csv(approved_mf_path)
