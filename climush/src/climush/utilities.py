...
def compress_data(input_path, output_path=None, compress_fmt='gzip', keep_input=False):
    ...
    :param input_path: path to the file or directory of files that contain either
...
def sort_taxonomy_info(
        input_df: pd.DataFrame,
        tax_column: str ='Taxonomy',
        drop_col: bool = True,
        tax_str_delim: str = ';',
    ) -> pd.DataFrame:
    '''
    ...
    :param tax_str_delim: the main taxonomy string delimiter that separates reference sequence match
    information (e.g., percent identity, reference sequence match accession number) from the taxonomic
    information (e.g., kingdom, order, genus)
    ...
    # column names for taxonomic levels
    tax_cols = ['kingdom', 'phylum', 'class', 'order', 'family', 'genus', 'species']

    # # create a dict where the key is the taxonomic level column label and the value is the one-letter code for this
    # #   taxonomic level in the SINTAX-formatted taxonomy string
    # tax_key = {tax_lvl: tax_lvl[0] for tax_lvl in tax_cols}

    ## MATCH INFO ##

    # new column names for the reference match info
    info_cols = ['tax_method', 'confidence', 'ref_01', 'ref_02']


    ## CREATE DICT FOR NEW COLUMN ROW VALUES ############

    # create a dictionary to store values for each of the new columns
    new_tax_cols = {col: [] for col in (info_cols + tax_cols)}


    ## ITERATE THROUGH TAXONOMY STRINGS #################

    # iterate through each row in the input_df[tax_column]
    for tax_str in dataframe[tax_column]:

        # check if this row lacks any taxonomic info
        if pd.isna(tax_str):

            # add empty values for the taxonomy string
            for tax in tax_cols:
                new_tax_cols[tax].append(pd.NA)

            # add empty values for the reference match columns
            for info in info_cols:
                new_tax_cols[info].append(pd.NA)

        # if this row does contain taxonomic info, continue...
        else:

            # split the taxonomy string by match info (confidence, reference nums) and taxonomy (kingdom, etc.)
            #   using the separate provided to the tax_str_delim function param
            tax_str_split = tax_str.split(tax_str_delim)

            ## SORT TAXONOMY STRING SECTIONS ####################

            # if the split taxonomy string has only one value...
            if len(tax_str_split) == 1:

                # the taxonomy string likely only has taxonomy info
                tax_info = tax_str_split[0]

                # create a match_info variable but leave as an empty string
                match_info = ''

            # if the split taxonomy string has two values...
            # the taxonomy string has both taxonomy information and reference match info
            elif len(tax_str_split) == 2:

                # the taxonomy string should have reference match info as the first value
                match_info = tax_str_split[0]

                # the taxonomy string should have taxonomy information as the second value
                tax_info = tax_str_split[1]

            # if the split taxonomy string doesn't have 1 or 2 values...
            else:
                taxstr_len_error = (f'The row value {tax_str} from the input data frame\'s {tax_column} column '
                                    f'has {len(tax_str_split)} sections, separated by a {tax_str_delim}, '
                                    f'when only 1 or 2 sections were expected.')
                return exit_process(taxstr_len_error)


            ## SORT TAXONOMIC LEVEL IDS #########################

            # iterate through each taxonomic level
            for tax_lvl in tax_cols:

                # get the taxonomic level indicator for this tax level, which is its first letter followed by
                #   a colon, assuming the taxonomy is in SINTAX format
                tax_lvl_indicator = f'{tax_lvl[0]}:'

                # create a regular expression to locate this taxonomic level by its taxonomic level indicator

                # tax level at start of string (kingdom) - at start (^) and followed by a comma (,)
                tax_loc_start = f'^{tax_lvl_indicator}(.+?)(?=,)'

                # tax level in middle of string - preceded by comma (,) and followed by comma (,)
                tax_loc_middle = f'(?<=,){tax_lvl_indicator}(.+?)(?=,)'

                # tax level at end of string - preceded by comma (,) and ends at end of string with only letters or
                #   white space between comma and end of string; this means there cannot be another comma between
                #   the preceding comma and end of the string
                tax_loc_end = f'(?<=,){tax_lvl_indicator}([A-Za-z ]+?)$'

                # combine the three possible regex into one with the or (|) operator
                tax_loc_all = '|'.join([tax_loc_start, tax_loc_middle, tax_loc_end])

                # search for this tax lvl at any location in the tax string
                tax_match = re.search(tax_loc_all, tax_info)

                # if a match is found for this taxonomic level...
                if tax_match:

                    # update the taxonomy dict with the matching taxonomy for this lvl
                    new_tax_cols[tax_lvl].append(tax_match.group(0))

                # if there was no match found...
                else:

                    # add an NA value for this taxonomic lvl
                    new_tax_cols[tax_lvl].append(pd.NA)


            ## SORT REFERENCE MATCH INFO ########################

            # if this taxonomy str doesn't have any reference match info...
            if match_info == '':
            ...
            # if this taxonomy str does have reference match info...
            else:

                ## MAKE REFERENCE INFO REGEX ########################

                # create a regular expression for the reference match column; leave ref02 empty as it can be a
                #  genbank ref number of UDB match
                ref_regex = {
                    reg : col for reg, col in \
                    zip(
                        info_cols,
                        [r'^[A-Z]{2,3}(?=|)', r'(?<=|)([0-9]{1,3}\.[0-9]{1,4})(?=|)', r'(?<=|)SH.+FU', r'(?<=|)[A-Z1-9](?=|)'],
                    )
                }

                # go through each regular expression...
                for new_col, regex in ref_regex.items():

                    # and try to match to an item in the ref match info
                    info_found = re.findall(regex, match_info)
                    ...
                    # if no match found, then add a pd.NA value for this
                    else:
                        new_tax_cols[new_col].append(pd.NA)
